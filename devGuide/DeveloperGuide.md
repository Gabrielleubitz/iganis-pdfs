# IGANI-PDF Developer Guide

## 1. Introduction

IGANI-PDF is a robust, locally hosted, web-based PDF manipulation tool. This guide focuses on Docker-based development and testing, which is the recommended approach for working with the full version of IGANI-PDF.

## 2. Project Overview

IGANI-PDF is built using:
...
1. Clone the repository:
   ...
   ```bash
   git clone https://github.com/Stirling-Tools/IGANI-PDF.git
   cd IGANI-PDF
   ```
...
4. Lombok Setup
IGANI-PDF uses Lombok to reduce boilerplate code. Some IDEs, like Eclipse, don't support Lombok out of the box. To set up Lombok in your development environment:
Visit the [Lombok website](https://projectlombok.org/setup/) for installation instructions specific to your IDE.

5. Add environment variable
For local testing, you should generally be testing the full 'Security' version of IGANI PDF. To do this, you must add the environment flag DISABLE_ADDITIONAL_FEATURES=false to your system and/or IDE build/run step.
...
IGANI-PDF offers several Docker versions:
...
IGANI-PDF provides several example Docker Compose files in the `exampleYmlFiles` directory, such as:
...
services:
  stirling-pdf:
    container_name: IGANI-PDF-Security
    ...
    environment:
      ...
      UI_APPNAME: IGANI-PDF
      UI_HOMEDESCRIPTION: Demo site for IGANI-PDF Latest with Security
      UI_APPNAMENAVBAR: IGANI-PDF Latest
...
IGANI-PDF uses different Docker images for various configurations. The build process is controlled by environment variables and uses specific Dockerfile variants. Here's how to build the Docker images:
...
IGANI-PDF provides a `test.sh` script in the root directory. This script builds all versions of IGANI-PDF, checks that each version works, and runs Cucumber tests. It's recommended to run this script before submitting a final pull request.
...
For quick iterations and development of Java backend, JavaScript, and UI components, you can run and test IGANI-PDF locally without Docker. This approach allows you to work on and verify changes to:
...
API documentation is available at `/swagger-ui/index.html` when running the application. You can also view the latest API documentation [here](https://app.swaggerhub.com/apis-docs/Stirling-Tools/IGANI-PDF/).

## 9. Customization

IGANI-PDF can be customized through environment variables or a `settings.yml` file. Key customization options include:
...
For managing language translations that affect multiple files, IGANI-PDF provides a helper script:
...
Thymeleaf is a server-side Java HTML template engine. It is used in IGANI-PDF to render dynamic web pages. Thymeleaf integrates heavily with Spring Boot.

### Thymeleaf overview

In IGANI-PDF, Thymeleaf is used to create HTML templates that are rendered on the server side. These templates are located in the `app/core/src/main/resources/templates` directory. Thymeleaf templates use a combination of HTML and special Thymeleaf attributes to dynamically generate content.
...
## Adding New Translations to Existing Language Files in IGANI-PDF

When adding a new feature or modifying existing ones in IGANI-PDF, you'll need to add new translation entries to the existing language files. Here's a step-by-step guide:
