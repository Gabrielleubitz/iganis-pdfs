...
main() {
    ...
    run_tests "IGANI-PDF-Ultra-Lite" "./exampleYmlFiles/docker-compose-latest-ultra-lite.yml"
    ...
    if verify_app_version "IGANI-PDF-Ultra-Lite" "http://localhost:8080"; then
        passed_tests+=("IGANI-PDF-Ultra-Lite-Version-Check")
        echo "Version verification passed for IGANI-PDF-Ultra-Lite"
    else
        failed_tests+=("IGANI-PDF-Ultra-Lite-Version-Check")
        echo "Version verification failed for IGANI-PDF-Ultra-Lite"
    fi

    docker-compose -f "./exampleYmlFiles/docker-compose-latest-ultra-lite.yml" down

    # run_tests "IGANI-PDF" "./exampleYmlFiles/docker-compose-latest.yml"
    ...
    # run_tests "IGANI-PDF-Ultra-Lite-Security" "./exampleYmlFiles/docker-compose-latest-ultra-lite-security.yml"
    # docker-compose -f "./exampleYmlFiles/docker-compose-latest-ultra-lite-security.yml" down
    # run_tests "IGANI-PDF-Security" "./exampleYmlFiles/docker-compose-latest-security.yml"
    # docker-compose -f "./exampleYmlFiles/docker-compose-latest-security.yml" down


    run_tests "IGANI-PDF-Security-Fat" "./exampleYmlFiles/docker-compose-latest-fat-security.yml"
    ...
    echo "Testing version verification..."
    if verify_app_version "IGANI-PDF-Security-Fat" "http://localhost:8080"; then
        passed_tests+=("IGANI-PDF-Security-Fat-Version-Check")
        echo "Version verification passed for IGANI-PDF-Security-Fat"
    else
        failed_tests+=("IGANI-PDF-Security-Fat-Version-Check")
        echo "Version verification failed for IGANI-PDF-Security-Fat"
    fi

    docker-compose -f "./exampleYmlFiles/docker-compose-latest-fat-security.yml" down

    run_tests "IGANI-PDF-Security-Fat-with-login" "./exampleYmlFiles/test_cicd.yml"
    ...
    if [ $? -eq 0 ]; then
        ...
        CONTAINER_NAME="IGANI-PDF-Security-Fat-with-login"
        ...
        if python -m behave; then
            ...
            if compare_file_lists "$BEFORE_FILE" "$AFTER_FILE" "$DIFF_FILE" "$CONTAINER_NAME"; then
                ...
                passed_tests+=("IGANI-PDF-Regression")
            else
                echo "WARNING: Unexpected temporary files detected after behave tests!"
                failed_tests+=("IGANI-PDF-Regression-Temp-Files")
            fi

            passed_tests+=("IGANI-PDF-Regression")
        else
            failed_tests+=("IGANI-PDF-Regression")
    ...
    run_tests "IGANI-PDF-Fat-Disable-Endpoints" "./exampleYmlFiles/docker-compose-latest-fat-endpoints-disabled.yml"
    ...
    echo "Testing version verification..."
    if verify_app_version "IGANI-PDF-Fat-Disable-Endpoints" "http://localhost:8080"; then
        passed_tests+=("IGANI-PDF-Fat-Disable-Endpoints-Version-Check")
        echo "Version verification passed for IGANI-PDF-Fat-Disable-Endpoints"
    else
        failed_tests+=("IGANI-PDF-Fat-Disable-Endpoints-Version-Check")
        echo "Version verification failed for IGANI-PDF-Fat-Disable-Endpoints"
    fi
